  ...
  BEGIN DECLARE PARTITIONDATE varchar(40); 
  ...
  SELECT week_date_id  INTO PARTITIONWEEKDATEID FROM dim_date WHERE date = PARTITIONDATE;  SELECT month_date_id INTO PARTITIONMONTHDATEID FROM dim_date WHERE date = PARTITIONDATE;  SELECT year_date_id  INTO PARTITIONYEARDATEID FROM dim_date WHERE date = PARTITIONDATE;   
  
  ALTER TABLE agg_event_resource_world_day TRUNCATE PARTITION current;  
  
  SET @sql := CONCAT("ALTER TABLE agg_event_resource_world_day REORGANIZE PARTITION current,previous INTO (
  PARTITION previous VALUES LESS THAN (",PARTITIONDATEID, "), PARTITION current VALUES LESS THAN MAXVALUE);");       
  PREPARE stmt FROM @sql;  
  EXECUTE stmt;  
  DEALLOCATE PREPARE stmt;  
  
  IF(PARTITIONDATEID = PARTITIONWEEKDATEID) THEN
      SET @sql := CONCAT("ALTER TABLE agg_event_resource_world_week REORGANIZE PARTITION current,previous INTO (
  ...
  END IF;
   ...
   IF(PARTITIONDATEID = PARTITIONMONTHDATEID) THEN
      SET @sql := CONCAT("ALTER TABLE agg_event_resource_world_month REORGANIZE PARTITION current,previous INTO (
  ...
  IF(PARTITIONDATEID = PARTITIONYEARDATEID) THEN
    SET @sql := CONCAT("ALTER TABLE agg_event_resource_world_year REORGANIZE PARTITION current,previous INTO (
