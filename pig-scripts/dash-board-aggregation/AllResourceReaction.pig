...
SET job.name 'agg_event_resource_reaction';
...
GROUPED_REACTION_VALUES = GROUP GET_REACTION_VALUES by (parent_gooru_oid,gooru_oid,DUPLICATE_REACTION_IDS::value,organization_uid);

FLATTENED_REACTION_VALUES = FOREACH GROUPED_REACTION_VALUES GENERATE StringConcat(((group.parent_gooru_oid.value) is null ? 'NA':group.parent_gooru_oid.value),'-',((group.organization_uid.value)is null ? 'NA':group.organization_uid.value)) as KEY, FLATTEN(GET_REACTION_VALUES) ,COUNT($1) as reaction_count;
