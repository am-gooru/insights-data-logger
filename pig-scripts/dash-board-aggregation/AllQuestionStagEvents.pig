...
STAGE_VALUES = FOREACH STAGE GENERATE key, hour_id,date_id,event_id,user_uid,gooru_oid,parent_gooru_oid,total_timespent_ms.value as total_timespent_ms, organization_uid,attempt_first_status,event_value,resource_type,app_key,app_id;

DISTINCT_STAGE_VALUES = DISTINCT STAGE_VALUES;

CQRP_EVENTS = FILTER DISTINCT_STAGE_VALUES   BY (event_id.value == 224);

GROUPED_VALUES = GROUP CQRP_EVENTS by (hour_id,date_id,event_id,gooru_oid,parent_gooru_oid,user_uid,attempt_first_status,organization_uid,event_value,app_id);

FLATTENED_VALUES = FOREACH GROUPED_VALUES GENERATE StringConcat(group.hour_id.value,'-',group.date_id.value,'-',group.event_id.value,'-' ,(group.gooru_oid.value is null ? 'NA':group.gooru_oid.value),'-',(group.parent_gooru_oid.value is null ? 'NA':group.parent_gooru_oid.value),'-',(group.user_uid.value is null ? 'NA':group.user_uid.value),'-',(group.attempt_first_status.value is null ? 'NA':group.attempt_first_status.value),'-',(group.organization_uid.value is null ? 'NA':group.organization_uid.value),'-',(group.event_value.value is null ? 'NA':group.event_value.value),'-',(group.app_id.value is null ? 'NA':group.app_id.value)) as KEY, FLATTEN(CQRP_EVENTS) , COUNT($1) as attempt_first_status_count ;
