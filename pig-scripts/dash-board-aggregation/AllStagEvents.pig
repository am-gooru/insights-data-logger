...
STAGE_VALUES = FOREACH STAGE GENERATE key, hour_id,date_id,event_id,user_uid,gooru_oid,parent_gooru_oid,total_timespent_ms.value as total_timespent_ms, organization_uid,event_value,resource_type,app_key,app_id;

DISTINCT_STAGE_VALUES = DISTINCT STAGE_VALUES;

GROUPED_VALUES = GROUP DISTINCT_STAGE_VALUES by (hour_id,date_id,event_id,gooru_oid,user_uid,organization_uid,event_value,app_id);

FLATTENED_VALUES = FOREACH GROUPED_VALUES GENERATE StringConcat(group.hour_id.value,'-',group.date_id.value,'-',group.event_id.value,'-' ,(group.gooru_oid.value is null ? 'NA':group.gooru_oid.value),'-',(group.user_uid.value is null ? 'NA':group.user_uid.value),'-',(group.organization_uid.value is null ? 'NA':group.organization_uid.value),'-',(group.event_value.value is null ? 'NA':group.event_value.value),'-',(group.app_id.value is null ? 'NA':group.app_id.value)) as KEY, FLATTEN(DISTINCT_STAGE_VALUES) , COUNT($1) as total_occurences , SUM(DISTINCT_STAGE_VALUES.total_timespent_ms) as total_timespent_ms;
